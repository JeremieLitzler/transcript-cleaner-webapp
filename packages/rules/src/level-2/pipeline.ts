...
export const LEVEL_2_PIPELINE: readonly RuleDefinition[] = [
  {
    id: 11,
    name: 'Remove the closing trailer',
    apply: rule11RemoveTrailer,
    languageAgnostic: false,
  },
  {
    id: 9,
    name: 'Remove an immediately repeated paragraph',
    apply: rule9RemoveDuplicates,
    languageAgnostic: true,
  },
  {
    id: 6,
    name: 'Join a paragraph ending in "Mr."',
    apply: rule6MrJoin,
    languageAgnostic: false,
  },
  {
    id: 2,
    name: 'Join a paragraph starting with "and"',
    apply: rule2AndJoin,
    languageAgnostic: false,
  },
  {
    id: 10,
    name: 'Join a paragraph starting with "Then"',
    apply: rule10ThenJoin,
    languageAgnostic: false,
  },
  {
    id: 8,
    name: 'Join "That" followed by a pronoun',
    apply: rule8ThatPronounJoin,
    languageAgnostic: false,
  },
  {
    id: 4,
    name: 'Join two consecutive "But" paragraphs',
    apply: rule4ButButJoin,
    languageAgnostic: false,
  },
  {
    id: 7,
    name: 'Join a verbless sentence (needs an LLM — no-op)',
    apply: rule7VerblessJoinLLM,
    languageAgnostic: false,
  },
  {
    id: 1,
    name: 'Remove a leading "And"',
    apply: rule1RemoveAnd,
    languageAgnostic: false,
  },
  {
    id: 3,
    name: 'Capitalise a paragraph starting in lowercase',
    apply: rule3CapitaliseFirst,
    languageAgnostic: true,
  },
  {
    id: 5,
    name: 'Capitalise the word after a "?"',
    apply: rule5CapitaliseAfterQuestion,
    languageAgnostic: true,
  },
];
...
export function ruleById(id: RuleId): RuleDefinition {
  ...
  if (!rule) {
    throw new RangeError(`No level-2 rule with id ${id}`);
  }
  return rule;
}

/** Whether a number is one of the eleven rule numbers. */
export function isRuleId(value: number): value is RuleId {
  return LEVEL_2_PIPELINE.some((rule) => rule.id === value);
}
